...
export default function UpdateMood(){
  ...
  const handleSubmit = async (event) => {
    ...
    const requestBody = {
      ...
      const response = await fetch('http://localhost:8080/api/mood/updateNewMood', {
      method: 'PUT',
      headers: {
    ...
    if (response.ok) {
      toast.success('Mood update successfully'); // Display toast message on successful save
    } else {
      toast.error('Failed to update mood. You can save mood directly using Home page.'); // Display error toast message on failure
    }
